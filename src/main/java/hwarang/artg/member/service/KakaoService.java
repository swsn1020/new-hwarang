...
//		String REDIRECT_URI = "http://localhost:8081/member/kakaoLogin";
	...
	public void scops() {
		String REDIRECT_URI = "http://localhost:8081/member/kakaoLogin";
		String APP_KEY = "bd90e2de25bc0633a8db2d8a7de772b3";
	...
	public Map<String, Object> delete(String access_token){
		...
		try {
			...
			System.out.println("kakao delete: " + kakaoInfo);
	...
	public Map<String, Object> delete2(String admin_key){
		String REQUEST_URL = "https://kapi.kakao.com/v1/user/unlink";
		HttpClient client = HttpClientBuilder.create().build();
		HttpPost post = new HttpPost(REQUEST_URL);
		post.addHeader("Authorization", "KakaoAK " + admin_key);
		post.addHeader("target_id_type=user_id&target_id=","");
		...
		try {
			...
			int responseCode = response.getStatusLine().getStatusCode();
			ObjectMapper mapper = new ObjectMapper();
			System.out.println("delete2 : " + response.getEntity().getContent());
			node = mapper.readTree(response.getEntity().getContent());
			
			System.out.println("kakao delete2: " + kakaoInfo);
